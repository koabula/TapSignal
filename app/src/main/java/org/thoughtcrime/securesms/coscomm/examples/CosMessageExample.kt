...
object CosMessageExample {
    ...
    fun createCosRequest(context: android.content.Context): CosRequest? {
        // 从配置存储获取COS配置
        val cosConfig = org.thoughtcrime.securesms.cos.CosConfigStorage.getConfig(context)
            ?: return null
    ...
    fun manageSubAccountPool(context: android.content.Context) {
        // 从配置获取COS信息
        val cosConfig = org.thoughtcrime.securesms.cos.CosConfigStorage.getConfig(context) ?: run {
            println("COS配置未找到")
            return
        }

        // 生成临时访问凭证
        val cosClient = org.thoughtcrime.securesms.cos.CosClientFactory.createClient(cosConfig)
        ...
        val subAccountEntry = org.thoughtcrime.securesms.coscomm.manager.SubAccountEntry.create(
            recipientId = "recipient-12345",
