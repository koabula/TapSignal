...
object CosSubUserManagerFactory {
    ...
    fun createManager(config: CosConfig, context: android.content.Context): CosSubUserManager {
        return when (config.provider) {
            CosConfig.Provider.AWS -> org.thoughtcrime.securesms.cos.aws.AwsSubUserManager(config)
            CosConfig.Provider.TENCENT -> org.thoughtcrime.securesms.cos.tencent.TencentSubUserManager(config, context)
        }
    }
}
