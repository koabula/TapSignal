...
class AwsIoTClient(
    ...
    suspend fun connect(): Result<String> = suspendCoroutine { continuation ->
        try {
            ...
            val mqttBrokerUrl = "ssl://$endpoint:8883"
            ...
            client.connect(options, null, object : IMqttActionListener {
                override fun onSuccess(asyncActionToken: IMqttToken?) {
                    Log.i(TAG, "Connected to AWS IoT Core: $endpoint")
                    isConnected.set(true)
                    _connectionState.value = ConnectionState.Connected
                ...
                override fun onFailure(asyncActionToken: IMqttToken?, exception: Throwable?) {
                    Log.e(TAG, "Failed to connect to AWS IoT Core", exception)
                    isConnected.set(false)
                ...
                if (client.isConnected) {
                    suspendCoroutine { continuation ->
                        client.disconnect(null, object : IMqttActionListener {
                            override fun onSuccess(asyncActionToken: IMqttToken?) {
                                Log.i(TAG, "Disconnected from AWS IoT Core")
                                isConnected.set(false)
