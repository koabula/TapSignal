...
import org.thoughtcrime.securesms.tap.provider.cos.cos.CosConfigStorage
// CamPoolManager已删除，使用SubAccountPoolManager
import org.thoughtcrime.securesms.tap.provider.cos.coscomm.data.*
import org.thoughtcrime.securesms.tap.provider.cos.coscomm.storage.CosChannelStorage
...
class CosRequestManager private constructor(private val context: Context) {
        ...
        try {
            ...
            if (cosResponse.accepted) {
                ...
                if (established) {
                    ...
                    try {
                        org.thoughtcrime.securesms.tap.provider.cos.coscomm.manager.CosDisconnectionManager.insertV2ModeEnabledMessage(context, standardizedSenderId)
                    } catch (e: Exception) {
                    ...
                    if (addSubAccountResult.isSuccess()) {
                        ...
                        // 6. 启动轮询服务
                        try {
                            val pollingManager = org.thoughtcrime.securesms.tap.provider.cos.coscomm.manager.CosPollingManager.getInstance(context)
    ...
    private fun shouldUsePermanentCredentials(durationType: CosDuration): Boolean {
        ...
        return when (credentialType) {
            org.thoughtcrime.securesms.tap.provider.cos.cos.CosCredentialType.PERMANENT -> true
            org.thoughtcrime.securesms.tap.provider.cos.cos.CosCredentialType.TEMPORARY -> false
            org.thoughtcrime.securesms.tap.provider.cos.cos.CosCredentialType.AUTO -> {
    ...
    private fun generatePermanentCredentials(cosConfig: CosConfig, durationType: CosDuration): CosAccessInfo? {
        return try {
            ...
            val subUserManager = org.thoughtcrime.securesms.tap.provider.cos.cos.CosSubUserManagerFactory.createManager(cosConfig, context)
            ...
            val subUserCredential = subUserManager.createSubUser(
                ...
                permissions = org.thoughtcrime.securesms.tap.provider.cos.cos.CosPermission.READ_ONLY // 对方只能读取
            )
    ...
    private fun sendSignalMessage(recipientId: String, signalMessage: CosSignalMessage): Boolean {
        return try {
            ...
            val cosMessageWithPrefix = org.thoughtcrime.securesms.tap.provider.cos.coscomm.processor.CosSignalMessageProcessor.COS_MESSAGE_PREFIX + messageJson
