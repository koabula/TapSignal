...
object CosV2ModeVerification {
    ...
    private fun verifyMessageSending(context: Context): ComponentStatus {
        ...
        try {
            ...
            val sendManager = org.thoughtcrime.securesms.tap.provider.cos.coscomm.manager.CosMessageSendManager.getInstance(context)
    ...
    private fun verifyMessageReceiving(context: Context): ComponentStatus {
        ...
        try {
            ...
            val messageProcessor = org.thoughtcrime.securesms.tap.provider.cos.coscomm.processor.CosMessageProcessor.getInstance(context)
            Log.d(TAG, "消息处理器创建成功")
