package org.thoughtcrime.securesms.tap.provider.cos.coscomm.service
...
import org.thoughtcrime.securesms.tap.provider.cos.cos.CosFileInfo
...
class CosPollingService(private val context: Context) {
    ...
    // 核心组件
    private val subAccountPoolManager = org.thoughtcrime.securesms.tap.provider.cos.coscomm.manager.SubAccountPoolManager.getInstance(context)
    ...
    private val cosMessageProcessor by lazy {
        org.thoughtcrime.securesms.tap.provider.cos.coscomm.processor.CosMessageProcessor.getInstance(context)
    }
    ...
    private fun processReceivedMessages(recipientId: String, messages: List<CosMessage>) {
        ...
        processingExecutor.submit {
            try {
                ...
                when (result) {
                    is org.thoughtcrime.securesms.tap.provider.cos.coscomm.processor.CosMessageProcessor.ProcessingResult.Success -> {
                    ...
                    is org.thoughtcrime.securesms.tap.provider.cos.coscomm.processor.CosMessageProcessor.ProcessingResult.NoNewMessages -> {
                        Log.d(TAG, "无新消息需要处理: recipientId=$recipientId")
                    }
                    is org.thoughtcrime.securesms.tap.provider.cos.coscomm.processor.CosMessageProcessor.ProcessingResult.Error -> {
