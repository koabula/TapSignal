...
class MessageSendRoutingManager private constructor(private val context: Context) {
    ...
    private fun isCosControlMessage(outgoingMessage: OutgoingMessage): Boolean {
        ...
        // 检查是否包含COS消息前缀
        val cosMessagePrefix = org.thoughtcrime.securesms.tap.provider.cos.coscomm.processor.CosSignalMessageProcessor.COS_MESSAGE_PREFIX
    ...
    private fun isCosV2Mode(recipientId: String): Boolean {
        return try {
            // 检查是否启用永久凭证
            val isPermanentCredentialEnabled = org.thoughtcrime.securesms.tap.provider.cos.cos.CosConfigStorage.isPermanentCredentialEnabled(context)
            ...
            if (channel?.status != org.thoughtcrime.securesms.tap.provider.cos.coscomm.data.ChannelStatus.ACTIVE) {
                return false
            }
