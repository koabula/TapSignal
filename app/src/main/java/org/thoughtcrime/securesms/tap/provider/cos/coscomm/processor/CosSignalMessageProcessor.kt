...
import org.thoughtcrime.securesms.tap.provider.cos.coscomm.manager.CosDisconnectionManager
...
class CosSignalMessageProcessor private constructor(private val context: Context) {
    ...
    private val notificationManager = org.thoughtcrime.securesms.tap.provider.cos.coscomm.ui.CosRequestNotificationManager.getInstance(context)
    ...
    private fun processDisconnectionMessage(senderId: String, messageJson: String): CosMessageProcessResult {
        ...
        return try {
            // 解析断开连接消息
            val messageSerializer = org.thoughtcrime.securesms.tap.provider.cos.coscomm.utils.CosMessageSerializer
            val parseResult = messageSerializer.deserializeSignalMessage(messageJson)

            if (parseResult is org.thoughtcrime.securesms.tap.provider.cos.coscomm.data.CosResult.Error) {
                Log.e(TAG, "解析断开连接消息失败: ${parseResult.exception.message}")
                return CosMessageProcessResult.failure("unknown", "解析断开连接消息失败")
            }

            val signalMessage = (parseResult as org.thoughtcrime.securesms.tap.provider.cos.coscomm.data.CosResult.Success).data
            ...
            handleResult.whenComplete { result, throwable ->
                when {
                    ...
                    result is org.thoughtcrime.securesms.tap.provider.cos.coscomm.data.CosResult.Success -> {
                    ...
                    result is org.thoughtcrime.securesms.tap.provider.cos.coscomm.data.CosResult.Error -> {
