...
import org.thoughtcrime.securesms.tap.provider.cos.cos.tencent.TencentSigner
import org.thoughtcrime.securesms.tap.provider.cos.cos.tencent.TencentStsXmlParser
import java.io.File
...
class TencentCosClient(private val config: CosConfig, private val context: Context? = null) : CosClient {
    ...
    override fun generateTemporaryAccessToken(directoryPath: String, durationMinutes: Int): CosAccessToken {
        return try {
            // 注意：腾讯云COS SDK本身不直接提供STS功能
            // 这里我们仍然需要使用STS API来获取临时凭证
            // 但我们可以使用腾讯云官方的STS SDK（如果有的话）
            ...
            okHttpClient.newCall(request).execute().use { resp ->
                if (!resp.isSuccessful) {
                    ...
                    throw RuntimeException("获取临时访问凭证失败: ${resp.code} - $errorBody")
                }

                val responseBody = resp.body?.string() ?: throw RuntimeException("STS API响应为空")
            ...
            throw RuntimeException("生成临时访问凭证失败: ${e.message}")
        }
    }
